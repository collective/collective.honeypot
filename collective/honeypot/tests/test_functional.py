...
import transaction
from collective.honeypot.testing import (
...
class StandardTestCase(BaseTestCase):
    ...
    def test_old_comment_empty(self):
        ...
        try:
            ...
            add_comment.click()
            self.assertEqual(
                self.browser.url, self.portal_url + "/doc/discussion_reply_form"
            )
        try:
        ...
        self.assertTrue(
            "Comment cannot be blank." in self.browser.contents
            or "Please submit a body." in self.browser.contents
        )
        self.assertEqual(len(self.mailhost.messages), 0)
...
class FixesTestCase(ProfileTestCase):
    ...
    def test_old_comment_post_no_honey(self):
        ...
        self.assertRaises(
            Forbidden, self.browser.post, self.portal_url + "/doc/discussion_reply", ""
        )
        self.assertEqual(len(self.mailhost.messages), 0)
